...
class FosfatosDetalleAction extends OnChangePropertyBaseAction{
    ...
    // jcAbsorbancia, jcMlMuestra, jcMgP, jcFosfatos
        ...
        if (jdAbsorbancia && cteN1 && cteN2)
            getView().setValue("jdMgP", 
                (jdAbsorbancia * cteN1 - cteN2).setScale(3, BigDecimal.ROUND_HALF_UP)
            )
        
        // =+E21*(1000/D21)
        BigDecimal jdMgP = (BigDecimal)getView().getValue("jdMgP")
        if (jdMgP && jdMlMuestra)
            getView().setValue("jdFosfatos", 
                (jdMgP*(1000/jdMlMuestra)).setScale(3, BigDecimal.ROUND_HALF_UP)
            )
        ...
        if (jcAbsorbancia && cteN1 && cteN2)
            getView().setValue("jcMgP", 
                (jcAbsorbancia * cteN1 - cteN2).setScale(3, BigDecimal.ROUND_HALF_UP)
            )
        
        // =+E21*(1000/D21)
        BigDecimal jcMgP = (BigDecimal)getView().getValue("jcMgP")
        if (jcMgP && jcMlMuestra)
            getView().setValue("jcFosfatos", 
                (jcMgP*(1000/jcMlMuestra)).setScale(3, BigDecimal.ROUND_HALF_UP)
            )
    }
